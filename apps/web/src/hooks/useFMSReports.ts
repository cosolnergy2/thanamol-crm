...
export function usePredictiveMaintenanceReport(projectId: string) {
  return useQuery({
    queryKey: FMS_REPORTS_QUERY_KEYS.predictiveMaintenance(projectId),
    queryFn: () =>
      apiGet<{ report: PredictiveMaintenanceReport }>(
        `/fms/reports/predictive-maintenance${buildQueryString({ projectId })}`
      ),
    enabled: Boolean(projectId),
    staleTime: 60 * 1000,
  })
