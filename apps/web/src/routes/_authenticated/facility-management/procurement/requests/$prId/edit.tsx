...
type StoredConditions = {
  ...
  timeline_progress?: string | null
  late_penalty_percent?: number | null
  insurance?: boolean
}

type StoredDocument = { url: string }
...
function parseStoredConditions(stored: StoredConditions | null | undefined): PRConditions {
  if (!stored) return INITIAL_CONDITIONS
  return {
    ...
    retention_percent: stored.retention_percent ?? '',
    retention_years: stored.retention_years ?? '',
    warranty: stored.warranty ?? false,
    warranty_years: stored.warranty_years ?? '',
    credit_terms_days: stored.credit_terms_days ?? '',
    timeline_start: stored.timeline_start ?? '',
    timeline_end: stored.timeline_end ?? '',
    timeline_progress: stored.timeline_progress ?? '',
    late_penalty_percent: stored.late_penalty_percent ?? '',
    insurance: stored.insurance ?? false,
  }
}
...
function PREditPage() {
  ...
  const updatePR = useUpdatePurchaseRequest(prId)
  ...
  useEffect(() => {
    if (!data?.pr || initialized) return
    const pr = data.pr
    setCompanyId(pr.company_id ?? '')
    setSiteId(pr.site_id ?? '')
    setUnitId(pr.unit_id ?? '')
    setRequiredDate(pr.required_date ? pr.required_date.split('T')[0] : '')
    setPurpose(pr.purpose ?? '')
    setPmScheduleId(pr.pm_schedule_id ?? '')
    ...
    const storedDocs = (pr.documents ?? []) as StoredDocument[]
    setDocuments(storedDocs.length > 0 ? storedDocs.map((d) => d.url) : [''])

    setConditions(parseStoredConditions(pr.conditions as StoredConditions))
    setInitialized(true)
  }, [data, initialized])
    ...
    return (
      <div className="py-12 text-center">
        <p className="text-slate-500 mb-4">
          This purchase request cannot be edited. Only DRAFT requests can be modified.
        </p>
        <Button
          variant="outline"
          onClick={() =>
  ...
  const canSave = items.some((i) => i.item_name.trim())
  ...
  async function handleSave() {
    await updatePR.mutateAsync(buildUpdatePayload())
    navigate({
      to: '/facility-management/procurement/requests/$prId',
      params: { prId },
    })
  ...
  const isBusy = updatePR.isPending || submitPR.isPending
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{pr.pr_number}</p>
        </div>
      </div>
                  ...
                  >
                    ...
                    <div className="col-span-6">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateInstallment(inst._key, 'milestone', e.target.value)}
