...
export const Route = createFileRoute(
  ...
  component: VendorContractEditPage,
})
...
function slaEntriesToRows(entries: SlaEntry[]): SlaRow[] {
  ...
  return entries.map((e) => ({
    ...
    penaltyPerDay: e.penaltyPerDay != null ? String(e.penaltyPerDay) : '',
  }))
}
...
function VendorContractEditPage() {
  ...
  useEffect(() => {
    if (data?.contract && !initialized) {
      const c = data.contract
      setTitle(c.title)
      setContractType(c.contract_type ?? '')
      setServiceCategory(c.service_category ?? '')
      ...
      setAlertDays(c.alert_days_before_expiry != null ? String(c.alert_days_before_expiry) : '30')
      setAutoRenew(c.auto_renew)
      setStatus(c.status as VendorContractStatus)
      setScope(c.scope ?? '')
      setSlaRows(slaEntriesToRows(Array.isArray(c.sla) ? (c.sla as SlaEntry[]) : []))
      setRateCardRows(rateCardEntriesToRows(Array.isArray(c.rate_card) ? (c.rate_card as RateCardEntry[]) : []))
      setInitialized(true)
    }
  }, [data, initialized])
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5 font-extralight font-mono">
            {c.contract_number}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label>Vendor</Label>
                <Input value={c.vendor.name} disabled className="bg-slate-50" />
              </div>
              <div>
        ...
        <div className="flex justify-end gap-3 pb-6">
          ...
          <Button
            type="submit"
            disabled={updateContract.isPending || !title}
