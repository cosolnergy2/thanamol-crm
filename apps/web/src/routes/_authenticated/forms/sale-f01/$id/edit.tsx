...
export const Route = createFileRoute('/_authenticated/forms/sale-f01/$id/edit')({
  component: SaleF01EditPage,
})
...
function SaleF01EditPage() {
  const { id } = Route.useParams()
  const navigate = useNavigate()
  const { data: req, isLoading } = useWarehouseRequirement(id)
  const updateReq = useUpdateWarehouseRequirement()
  ...
  useEffect(() => {
    if (!req) return
    const reqs = req.requirements as Record<string, unknown>
    const specs = req.specifications as Record<string, unknown>
    reset({
      customerId: req.customer_id,
      projectId: req.project_id ?? undefined,
      status: req.status as FormValues['status'],
      warehouseSpaceSqm: reqs.warehouseSpaceSqm != null ? Number(reqs.warehouseSpaceSqm) : undefined,
      ceilingHeightM: reqs.ceilingHeightM != null ? Number(reqs.ceilingHeightM) : undefined,
      rentalPeriodYear: reqs.rentalPeriodYear != null ? Number(reqs.rentalPeriodYear) : undefined,
      notes: specs.notes ? String(specs.notes) : undefined,
    })
  }, [req, reset])
  ...
  async function onSubmit(values: FormValues) {
    try {
      ...
      navigate({ to: '/forms/sale-f01/$id', params: { id } })
    } catch (err) {
  ...
  return (
    <div className="space-y-3 max-w-4xl">
      <div className="flex items-center space-x-3 mb-4">
        <Link to="/forms/sale-f01/$id" params={{ id }}>
