...
import { useCreateSaleJob } from '@/hooks/useSaleJobs'
import type { CreateSaleJob04F01Request, SaleJobStatus } from '@thanamol/shared'

export const Route = createFileRoute('/_authenticated/forms/sale-job04-f01/new')({
  component: SaleJob04F01NewPage,
})
...
function SaleJob04F01NewPage() {
  const navigate = useNavigate()
  const createJob = useCreateSaleJob()
  ...
  async function onSubmit(values: FormValues) {
    const payload: CreateSaleJob04F01Request = {
    ...
    try {
      ...
      toast.success('Sale job created successfully')
      navigate({ to: '/forms/sale-job04-f01' })
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to create sale job')
    }
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <CardContent className="space-y-4 pt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="projectId" {...register('projectId')} placeholder="Project ID" />
                {errors.projectId && (
              ...
              <div className="space-y-2">
                ...
                <Input id="customerId" {...register('customerId')} placeholder="Customer ID" />
                {errors.customerId && (
              ...
              <div className="space-y-2">
                ...
                <Input id="unitId" {...register('unitId')} placeholder="Unit ID (optional)" />
              </div>
        ...
        <div className="flex items-center justify-end space-x-3 pb-8">
          ...
          >
            ...
            {createJob.isPending ? 'Saving...' : 'Save Sale Job'}
          </Button>
        </div>
      </form>
    </div>
