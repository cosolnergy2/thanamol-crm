...
export const fmsAssetCategoriesRoutes = new Elysia({ prefix: '/api/fms/asset-categories' })
  .use(authPlugin)
  .guard(
    {
    ...
    (app) =>
      app
        ...
        .get('/:id', async ({ params, set }) => {
          ...
          if (!category) {
            set.status = 404
            return { error: 'Asset category not found' }
          }
          return { category }
        })
        .post(
        ...
        .put(
          ...
          async ({ params, body, set }) => {
            const existing = await prisma.assetCategory.findUnique({ where: { id: params.id } })
            if (!existing) {
              set.status = 404
              return { error: 'Asset category not found' }
            }
            ...
            if (body.code && body.code !== existing.code) {
              const codeConflict = await prisma.assetCategory.findUnique({
                where: { code: body.code },
              })
              if (codeConflict) {
                set.status = 409
        ...
        .delete('/:id', async ({ params, set }) => {
          ...
          if (category._count.assets > 0) {
            set.status = 409
            return { error: 'Cannot delete category with assigned assets. Reassign assets first.' }
          }
